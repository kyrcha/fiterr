...
//		int generations = 1;
		...
		for(int r = 0; r < runs; r++) {
			System.out.println("*** " + (r + 1) + " th Experiment for NEAR and Mackey-Glass Begins!");
			...
			// The original sequence of Mackey-Glass
			...
			for(int gen = 1; gen <= generations; gen++) {
				...
				for(int pop = 0; pop < near.getPopulationSize(); pop++) {
					...
					totalNRMSEs /= J;
					totalNRMSEs = Utils.round(totalNRMSEs, 15);
					if(minTestNRMSE > totalNRMSEs) {
						minTestNRMSE = totalNRMSEs;
						individual = pop;
					}
					genome.setFitness(Utils.round(1.0/totalNRMSEs, 15));
				}
