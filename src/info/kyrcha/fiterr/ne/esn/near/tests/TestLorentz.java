...
//		int generations = 1;
		...
		for(int r = 0; r < runs; r++) {
			System.out.println("*** " + (r + 1) + " th Experiment for NEAR and Lorentz Begins!");
			...
			double[][] original = lorentz.createSequence(3020);
			...
			double originalMean = m.evaluate(original[0]);
			double originalMax = max.evaluate(original[0]);
			double originalMin = min.evaluate(original[0]);
			
			System.out.println(originalMean + " " + originalMax + " " + originalMin);
			
			for(int i = 0; i < original[0].length; i++) {
				original[0][i] = (original[0][i] - originalMean) / (originalMax - originalMin);
			}
			...
			for(int i = 0; i < (W + T); i++) {
				inputSequence[i][0] = original[0][i];
				inputSequence[i][1] = 1.0;
				if(i >= W) uTrain[i - W] = original[0][i];
			}
			...
			for(int i = 0; i < (W + T); i++) {
				outputSequence[i][0] = original[0][i+1];
//				System.out.println(outputSequence[i][0]);
			}
...
//						System.out.println(out + " " + original[i+1]);
						sum += Math.pow((out - original[0][i+1]), 2.0);
					} else if(i >= W) {
						sum2 += Math.pow((out - original[0][i+1]), 2.0);
						msq += Math.pow((out - original[0][i+1]), 2.0);
					}
				}
