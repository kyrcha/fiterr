 ...
 */
public class NEDPBGenAgent implements AgentInterface {
	
	/** Logger named after the class */
	private static final Logger logger = Logger.getLogger(NEDPBGenAgent.class.getName());
    ...
    private double[][]trace = new double[1000][4];
    
    private int traceCounter = 0;
    
    private boolean enableTrace = false;
    
    public NEDPBGenAgent(String fileName) {
...
//    	System.out.println(input.length + " " + observation.getNumDoubles());
...
//    	System.out.print("\n");
    ...
    public String agent_message(String message) {
    	if(message.equalsIgnoreCase("enable-trace")) {
    		enableTrace = true;
        	traceCounter = 0;
    		return null;
    	} else if(message.equalsIgnoreCase("disable-trace")) {
    		enableTrace = false;
    		return null;
    	} else if(message.equalsIgnoreCase("enable-output")) {
    		enableOutput = true;
    		return null;
    	...
    	} else if(message.equalsIgnoreCase("get-trace")) {
        		StringBuilder sbnet = new StringBuilder();
        		for(int i = 0; i < 1000; i++) {
        			for(int j = 0; j < 4; j++) {
        				sbnet.append(trace[i][j]);
        				sbnet.append(';');
        			}
        		}
        		sbnet.trimToSize();
    ...
    protected double[] encodeInput(Observation obsv) {
    	int length = 3;
    	if(enableTrace) {
    		trace[traceCounter][0] = obsv.getDouble(0);
    		trace[traceCounter][1] = obsv.getDouble(1);
    		trace[traceCounter][2] = obsv.getDouble(2);
    		trace[traceCounter][3] = obsv.getDouble(3);
    		traceCounter++;
    	}
    	if(enableOutput) {
...
//    	int counter = 1;
		...
		int counter = 0;
    	for(int i = 0; i < obsv.getNumDoubles(); i+=2) {
...
//    			System.out.print(encodedInput[counter] + " ");
    	...
    	for(int i = 0; i < obsv.getNumInts(); i+=2) {
    		if(network.getHiddenLayerFunction().compareTo(Function.TANH) == 0) {
